...
export function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <SidebarProvider
      style={
        { "--sidebar-width": "16rem", "--sidebar-width-icon": "4.5rem" } as React.CSSProperties
      }
    >
      <div className="flex min-h-screen w-full bg-background text-foreground">
        <Sidebar variant="sidebar" collapsible="icon" className="border-r border-sidebar-border">
          <SidebarHeader className="bg-sidebar h-16 px-4 flex items-center border-b border-sidebar-border">
            <Link href="/dashboard" className="flex items-center gap-2 w-full overflow-hidden">
          ...
          <SidebarContent className="bg-sidebar overflow-y-auto py-4">
            ...
            <div className="px-2 mb-6 hidden group-data-[collapsible=icon]:flex justify-center">
              <Tooltip>
                <TooltipTrigger asChild>
                  <Link
                    href="/add-expense"
                    className="p-2.5 gradient-primary text-primary-foreground rounded-lg shadow-md"
                  >
            ...
            <nav className="flex-1 space-y-0.5">
              {navItems.map((item) => {
                ...
                return (
                  <div key={item.title} className="group-data-[collapsible=icon]:px-1">
                    <div className="group-data-[collapsible=icon]:hidden">
                      <NavLink item={item} collapsed={false} active={active} />
                    </div>
                    <div className="hidden group-data-[collapsible=icon]:block">
                      <NavLink item={item} collapsed={true} active={active} />
                    </div>
                  </div>
                )
              })}
        ...
        <div className="flex flex-col flex-1 w-full min-w-0">
          <header className="h-16 flex items-center justify-between px-4 sm:px-6 lg:px-8 border-b border-card-border bg-card/80 backdrop-blur-md sticky top-0 z-30">
            <div className="flex items-center gap-3 flex-1 min-w-0">
              <SidebarTrigger className="hover-elevate active-elevate-2 bg-muted text-foreground rounded-md" />
